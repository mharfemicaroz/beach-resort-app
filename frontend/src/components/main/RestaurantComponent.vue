  ...
  <div class="container-fluid main">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
      ...
      <li
        ...
        role="presentation"
        v-if="
          userdata.role !== 'foodserver' && userdata.role !== 'restoinventory'
        "
      >
        <button
          :class="
      ...
      <li
        ...
        role="presentation"
        v-if="
          (userdata.role !== 'cashier' && userdata.role === 'foodserver') ||
      ...
      >
        <button
          :class="
            userdata.role === 'foodserver' ? 'nav-link active' : 'nav-link'
          "
          id="orders-tab"
          data-bs-toggle="tab"
      ...
      <li
        ...
        role="presentation"
        v-if="
          userdata.role === 'superuser' || userdata.role === 'restoinventory'
        "
      >
        <button
          :class="
            userdata.role === 'restoinventory' ? 'nav-link active' : 'nav-link'
          "
          id="inventory-tab"
          data-bs-toggle="tab"
    ...
    <div class="tab-content mt-3" id="myTabContent">
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="row row-cols-1 row-cols-md-6">
                      ...
                      <template
                        v-for="(item, index) in filteredresto_takeouts"
                        :key="item.id"
                      >
                        <div class="col mb-6">
                          <div
                          ...
                          >
                            ...
                            <div class="card-body" @click="takeoutAction(item)">
                              <h6 class="text-dark">
                                <i class="fas fa-info-circle"></i>
                                {{
                                  "order_id" in item ? "on progress" : "on hold"
                                }}
                              </h6>
                            </div>
                          </div>
              ...
              <div class="container-fluid">
                <div class="row">
                  <div class="col-md-12">
                    <div class="row row-cols-1 row-cols-md-6">
                      <template
                        v-for="(item, index) in filteredresto_onholds"
                        :key="item.id"
                      >
                        <div class="col mb-6" v-if="'order_id' in item">
                          ...
                          >
                            ...
                            >
                              <h5 class="card-title">
                                <i class="fa fa-dollars"></i> {{ item.name }}
                              </h5>
                            </div>
        ...
        <div class="row mt-2" id="posTab">
          <div class="col-md-8">
            <div class="row">
              <div class="col-lg-6 col-sm-6">
                <h4>
                  {{ userdata.role !== "waiter" ? "Point of Sales" : "Menu" }}
                  <span v-if="customer.reference_id !== null">
                    >>>&NonBreakingSpace;
                    <span
                      class="blink_me text-danger"
                      style="font-style: italic"
                      >Now serving: {{ customer.identifier }} for
                      {{ customer.type }}</span
                    >
                  </span>
                </h4>
            ...
            <div class="row mt-3 d-flex justify-content-center">
              ...
              <button
                :disabled="
                  cartItems.length < 1 || customer.reference_id !== null
                "
                v-if="userdata.role !== 'waiter'"
                class="btn btn-outline-primary btn-block btn-box btn-gap"
                @click="holdCustomer"
              ...
              <button
                :disabled="
                  cartItems.length < 1 || customer.order_id !== undefined
                "
                class="btn btn-outline-primary btn-block btn-box btn-gap"
                @click="clearAll"
              >
          ...
          <div class="col-md-4">
                        ...
                        <td>
                          <div class="row d-flex flex-row">
                            ...
                            <div class="col-sm-8 text-left m-0 p-0">
                              <h6 class="text-left" style="font-weight: bold">
                                {{ item.name }}
                              </h6>
                            </div>
                          </div>
            ...
            <div class="box mt-2" v-if="userdata.role !== 'waiter'">
              <div
                class="row bg-primary text-white d-flex flex-row-reverse align-items-center"
              >
                <div class="col-md-8">
                  <div class="input-group">
                    <span
              ...
              <div
                :class="
                  totalChange < 0
                    ? 'row mt-2 mb-2 bg-danger text-white'
                    : 'row mt-2 mb-2 bg-success text-white'
                "
                v-if="userdata.role !== 'waiter'"
              >
                <div class="col-md-6">
      ...
      <div
        :class="
          userdata.role !== 'foodserver' && userdata.role !== 'waiter'
            ? 'tab-pane fade'
            : 'tab-pane fade show active'
        "
        id="orders"
          ...
          <div class="col-md-12">
            <div class="row">
              <div class="col-md-12">
                <div class="row row-cols-1 row-cols-md-4">
                  <template
                    v-for="(item, index) in filteredResto_allorders"
                    :key="item.id"
                  >
                    <div
                      class="col mb-4"
                      v-if="
                        item.order_items.filter(
                          (o) =>
                            o.category.toLowerCase() !== 'drinks' &&
                            o.category.toLowerCase() !== 'miscellaneous'
                        ).length > 0
                      "
                    >
                      <div
                      ...
                      >
                        ...
                        >
                          <h5 class="card-title">
                            #{{ Number(item.id).toString().padStart(5, "0") }}
                          </h5>
                        ...
                        <div class="card-body">
                          <ul style="list-style-type: none; padding-left: 20px">
                            {{
                              item.order_type.toString().toUpperCase()
                            }}/{{
                              item.customer_name
                            }}
            ...
            <div class="tab-content">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-10">
                    <table-component
                      ...
                      :mainItems="superfilteredTransactions"
                      :subHeaders="transactionitem"
                      :toggleable="true"
                      :deletable="true"
                      @delete-action="voidItem"
                    />
                  </div>
                </div>
              </div>
              <div id="byitems" class="tab-pane">
                <div class="row">
                  ...
                  <div class="col-sm-10">
                    <table-component
                      :mainHeaders="transactionordersoptions"
                      :mainItems="filteredtransactionorders"
                      :subHeaders="transactionitem"
                      :toggleable="true"
                    />
                  </div>
  ...
  <div style="display: none">
    <div class="receipt" id="billing-statement">
      <div style="text-align: center">
        <span style="font-size: 18px; font-weight: bold">{{ APP_NAME }}</span
        ><br />
      ...
      <div style="margin-top: 10px">
        <table style="width: 100%; margin-top: 10px">
          ...
          <tr>
            ...
            <td>
              {{
                this.customer.order_id === undefined
                  ? "N/A"
                  : this.customer.order_type + "/" + this.customer.order_id
              }}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td>
              ₱{{
                ((parseFloat(taxValue) / 100) * parseFloat(subTotal)).toFixed(2)
              }}
            </td>
          </tr>
          <tr>
            ...
            <div class="col-md-6">
              <div v-if="stock">
                <img
                  :src="stock.imageUrl"
                  alt="Stock Image"
                  class="img-fluid mb-2 w-75 h-75"
                  v-if="stock.imageUrl"
                />
                <h5>{{ stock.name }}</h5>
...
export default {
  ...
  computed: {
    ...
    filteredResto_allorders() {
      return this.resto_allorders.filter(
        (item) =>
          new Date(item.date_created).setHours(0, 0, 0, 0) ===
    ...
    totalCost() {
      return (
        this.discountType === "percentage"
          ? this.subTotal *
            (1 + this.taxValue / 100) *
            (1 - this.discountValue / 100)
          : this.subTotal * (1 + this.taxValue / 100) - this.discountValue
      ).toFixed(2);
    },
    totalChange() {
    ...
    filteredresto_takeouts() {
      return this.resto_takeouts
        .map((item) => {
          const order = this.resto_order.filter(
            (o) => o.table_id === item.id && o.order_type === "take-out"
          );
          if (order.length > 0) {
    ...
    filteredresto_onholds() {
      return this.resto_onholds
        .map((item) => {
          const order = this.resto_order.filter(
            (o) => o.table_id === item.id && o.order_type === "on-hold"
          );
          if (order.length > 0) {
            const order_data = order[0];
            const order_id = order_data.id;
    ...
    superfilteredTransactions() {
      ...
      if (
        ...
        filtered = filtered.filter((transaction) => {
          return (
            parseDate2(transaction.date_created) >=
              parseDate(this.resfromDate) &&
            parseDate2(transaction.date_created) <= parseDate(this.restoDate)
          );
        });
      }
  ...
  methods: {
    ...
    async saveImageFile() {
      try {
        ...
        const response = await axios.post(
          this.API_URL + "restoitem/savefile",
          formData
        );
    ...
    async doneServe(item) {
      ...
      if (customer_id !== null) {
        const res = await axios.post(`${this.API_URL}restoorders/filter/`, {
          columnName: "id",
          columnKey: customer_orderId,
        });
        const existingOrder = res.data;
        axios.put(`${this.API_URL}restoorders/${existingOrder[0].id}/`, {
        ...
        this.$swal
          ...
          .then((result) => {
            if (result.isConfirmed) {
              ...
              axios
                .post(`${this.API_URL}restoonholds/`, {
                  name: inputValue,
                  status: "on hold",
                })
                .then((response) => {
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            axios
              .get(this.API_URL + `restotakeouts/delete/${id}/`)
              .then((response) => {
                this.getRestoTakeout();
                this.$swal.fire(
                  "Deleted!",
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            const inputValue = result.value;
            axios
              .post(`${this.API_URL}restotakeouts/`, {
                name: inputValue,
                status: "on process",
              })
              .then((response) => {
          ...
          if (result.isConfirmed) {
            ...
            if (customer_id !== null) {
              const res = await axios.post(
                `${this.API_URL}restoorders/filter/`,
                { columnName: "id", columnKey: customer_orderId }
              );
              const existingOrder = res.data;
              axios.put(`${this.API_URL}restoorders/${existingOrder[0].id}/`, {
                order_type: customer_type,
              ...
              for (const item of this.cartItems) {
                ...
                axios.put(`${this.API_URL}restoitem/${item.id}/`, {
                  sku: item.sku,
                  name: item.name,
    ...
    async placeOrder() {
      ...
      if (result.isConfirmed) {
        const countdownMessage =
          'Order will be saved in <span id="countdown">5</span> seconds. Do you want to cancel?';
        let countdownResult;
        countdownResult = await this.$swal.fire({
          title: "Please wait",
          html: countdownMessage,
        ...
        if (customer_id !== null) {
          if (
            ...
            customer_type === "take-out" ||
            customer_type === "on-hold"
          ) {
            const res = await axios.post(`${this.API_URL}restoorders/filter/`, {
              columnName: "id",
              columnKey: customer_orderId,
            ...
            if (existingOrder.length > 0) {
              for (const item of this.cartItems ||
                this.cartItems.filter(
                  (item) => !this.customer.items.includes(item)
                )) {
                const items = item.inventory;
    ...
    addToCash(d) {
      this.totalCash = isNaN(parseFloat(this.totalCash))
        ? 0
        : parseFloat(this.totalCash) + d;
    },
    clearAll() {
    ...
    async payOrder() {
      ...
      if (result.isConfirmed) {
        ...
        if (this.customer.reference_id !== null) {
          axios.put(`${this.API_URL}restoorders/${this.customer.order_id}/`, {
          ...
          if (this.customer.type === "take-out") {
            axios.put(
              `${this.API_URL}restotakeouts/${this.customer.reference_id}/`,
              {
            ...
            axios.put(
              `${this.API_URL}restoonholds/${this.customer.reference_id}/`,
              {
                name: this.customer.identifier,
                status: "done",
              }
    ...
    getAllOrders() {
      axios
        ...
        .then((response) => {
          this.resto_allorders = response.data
            .map((item) => {
              const timePassed = this.getTimePassed(item.datestarted);
              const isRunning = item.status === "progress" ? true : false;
    ...
    saveInventory() {
      if (this.isUpdatingInventory) {
        axios
          .get(`${this.API_URL}restoitem/${this.stock.id}/`)
          .then((response) => {
    ...
    addItemToCart(item) {
      const isItemExist =
        this.cartItems.filter((o) => o.id === item.id).length > 0;
      const cart = {
        id: item.id,
        justadded: true,
        sku: item.sku,
    ...
    printSection() {
      ...
      printWindow.document.write(
        "<style>@media print { body { font-size: 10pt; padding:0; margin: 0cm; padding: 0; width: 58mm; transform: scale(0.97); transform-origin: 0 0; } tr { page-break-inside: auto; } .no-print { display: none; } }</style>"
      );
    ...
    handleKeyPress(event) {
      ...
      switch (event.key) {
        case "F1":
          ...
          if (this.cartItems.length >= 1) this.placeOrder();
          break;
        case "F2":
          event.preventDefault();
          if (
            this.cartItems.length >= 1 &&
            this.userdata.role !== "waiter" &&
            this.userdata.role !== "foodserver"
          )
            this.payOrder();
          break;
        case "F3":
        ...
        case "F6":
          ...
          if (
            ...
            this.userdata.role !== "waiter" &&
            this.userdata.role !== "foodserver" &&
            this.customer.reference_id === null
          )
            this.holdCustomer();
          break;
        case "F7":
      ...
      if (e.keyCode === 13) {
        if (barcode.length > 10) {
          if (!this.inquiretoggle) {
            ...
            const itemlist = this.filtereditemarray.filter((item) =>
              item.sku.toString().toLowerCase().includes(this.barcodeText)
            );
            if (itemlist.length === 1) {
              ...
              if (this.userdata.role === "restoinventory") {
                const stock = this.itemarray.find(
                  (item) => item.id === itemlist[0].id
                );
                this.stock = stock;
                $("#stockModal").modal("toggle");
              }
