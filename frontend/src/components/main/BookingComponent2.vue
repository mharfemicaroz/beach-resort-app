<template :key="componentKey">
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="booking-tab" data-bs-toggle="tab" data-bs-target="#booking"
                    type="button" role="tab" aria-controls="booking" aria-selected="true" @click="resetSummary(1)">
                    Calendar
                </button>
            </li>
            ...
            <li v-if="userdata.role !== 'reservationist'" class="nav-item" role="presentation">
                <button class="nav-link" id="others-tab" data-bs-toggle="tab" data-bs-target="#others" type="button"
                    role="tab" aria-controls="others" aria-selected="false" @click="resetSummary(2)">
                    Account
                </button>
            </li>
            <li v-if="userdata.role === 'superuser'" class="nav-item" role="presentation">
                        ...
                        <div class="col-md-12">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="form-outline col-md-3">
                                    <input type="search" id="form1" class="form-control" placeholder="Type query"
                                        v-model="booksearchtext" @input="populateCalendarItems"
                                        @click="showAutosuggestions = false" @blur="hideAutosuggestions" aria-label="Search"
                                        ref="searchQuery" autocomplete="off" aria-autocomplete="off" />
                                    <ul id="suglist" class="autosuggestions" v-if="!showAutosuggestions">
                                        <li v-for="suggestion in autosuggestions" @click="selectSuggestion(suggestion)">
                                            {{ suggestion }}
                                        </li>
                                    </ul>
                                </div>
                            ...
                            <div class="calendar-parent c-p">
                                <calendar-view :items="calendarItems" :show-date="showDate"
                                    ...
                                    :displayWeekNumbers="displayWeekNumbers" :enable-date-selection="true"
                                    ...
                                    <template #header="{ headerProps }">
                                        <calendar-view-header slot="header" :header-props="headerProps"
                                            @input="setShowDate" />
                                    </template>
                                </calendar-view>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-sm-2">
                            <ul class="nav nav-tabs flex-column" id="propertyTab" role="tablist">
                                <li class="nav-item" v-for="(category, index) in roomcategories" :key="category.id">
                                    <a :class="index > 0 ? 'nav-link' : 'nav-link active'" data-bs-toggle="tab"
                                        @click="activeMainTab = `${category.name}`" :href="`#roomcategory${index}`">{{
                                            category.name }}</a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-sm-10">
                            <div class="tab-content mt-3" id="propertyTabContent">
                                <div v-for="(category, index) in roomcategories" :key="category.id" :class="index > 0 ? 'tab-pane fade' : 'tab-pane fade show active'
                                    " :id="`#roomcategory${index}`" role="tabpanel">
                                    <div class="container-fluid">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <CardBookingsVue :roomData="roomsjoinbookings"
                ...
                <div class="container-fluid">
                    <div class="row">
                        <ReceptionHotel :bookingsdata="bookings" :roomsdata="rooms" :roomcategoriesdata="roomcategories"
                            ...
                            @dragend-action="handledragend" @dragover-action="handleDragOver"
                      ...
                      <span><strong>Address:</strong> {{ billing.clientAddress }}</span>
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="row justify-content-between">
                                                <div class="col-4">
                                                    <img :src="`/src/assets/${this.APP_LOGO_NAME}`" width="60" height="60"
                                                            ...
                                                            <td v-if="item.itemOption !== 'room'">
                                                                ...
                                                                <td v-else>
                                                                    <span v-if="!isNaN(subroom.discountValue)" v-html="`${item.totalCartPrice
                                                                        } <sup class='text-danger font-weight-bold'>${subroom.discountMode === 'percentage'
                                                                        ...
                                                                        } off</sup> <span class='text-success font-weight-bold'>${subroom.discountMode === 'percentage'
                                                                            ? item.totalCartPrice *
                                                                            (1 -
                                                                                parseFloat(
                        ...
                        <div class="col-md-3"
                            ...
                            <div class="d-flex align-items-center" style="padding: 10px">
                                ...
                                <div class="ms-auto d-flex align-items-center">
                                    <!-- Wrap the buttons in a flex container -->
                                    <button type="button" class="btn btn-primary" @click="initializePlaceOrder"
                                        :disabled="total <= 0 || countInclusion > 0" style="
                      display: flex;
                      ...
                      <div class="col-sm-4">
                        <input type="number" class="form-control" id="discountValue" v-model="discountValue">
                                            ...
                                            <div class="form-group mb-2 mt-0">
                                                <div class="input-group">
                                                    <input type="text" class="form-control" :value="this.billing.clientName"
                                                        readonly />
                                                    <button type="button" class="btn btn-primary"
                                                        @click="toggleAddAccountModal">
    ...
    discountMode === "percentage" ? "%" : " off"
                                                    ...
                                                    <div class="row mb-0">
                                                        ...
                                                        <div class="col-6 text-right">
                                                            <strong>{{ total }}</strong>&nbsp;
                                                            <a v-if="total < 0" href="#" @click="refundPayment"><i>for
                                                                    refund→</i></a>
                                                        </div>
                                                    </div>
                                                    <!-- <div class="row">
                                            ...
                                            <div class="form-group">
                                                <div class="input-group">
                                                    ...
                                                    <input type="number" class="form-control" id="cashAmount"
                                                        v-model.number="cashAmount" step="0.01" @keyup="updateTotalCash"
                                            ...
                                            <div class="row mt-2 mb-2">
                                                <div class="col-md-12">
                                                    <div class="row row-cols-1 row-cols-md-4">
                                                        <div class="col mb-1" v-for="item in cashDenominations"
                                                            :key="item.id">
                                                            <button type="button"
                                ...
                                @click="printReservationHistory">Print</button>&nbsp;
                                            ...
                                            <div class="col-md-9">
                                                <div id="reservationHistory">
                                                    ...
                                                    <table-component :mainHeaders="reservationsOptions"
                                                        :mainItems="filteredReservationsHistory" :editable="false"
                                                        :toggleable="false" />
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-md-10">
                                                <div id="transactionHistory">
                                                    ...
                                                    <table-component :mainHeaders="transactionsOptions"
                                                        :mainItems="filteredTransactions" :subHeaders2="transactionhistory"
                                                        :subHeaders="transactionitem" @delete-action="deleteTransaction"
                                                        :deletable="true" :editable="false" :toggleable="true" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="col-12">
                        <div class="row justify-content-between" :style="!isThereLeisures ? 'border-right: dotted;' : ''">
                            ...
                            <div class="col-4 text-right">
                                <span style="font-size: small">Registration No.: {{ this.billing.bookingID }}</span>
                            </div>
                        </div>
                        <hr />
                    </div>
                ...
                <div class="modal-body">
                    <ul class="nav nav-tabs" id="roomTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="cancelled-tab" data-bs-toggle="tab" data-bs-target="#cancelled"
                                type="button" role="tab" aria-controls="cancelled" aria-selected="true"
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="reserved-tab" data-bs-toggle="tab" data-bs-target="#reserved"
                                type="button" role="tab" aria-controls="reserved" aria-selected="true"
                                @click="activeTab = 'reserved'">
                                Reserved
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="occupied-tab" data-bs-toggle="tab" data-bs-target="#occupied"
                                type="button" role="tab" aria-controls="occupied" aria-selected="false"
                                @click="activeTab = 'occupied'">
                                Occupied
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="checkedout-tab" data-bs-toggle="tab" data-bs-target="#checkedout"
                                type="button" role="tab" aria-controls="checkedout" aria-selected="false"
                    ...
                    <tbody>
                      <tr v-for="reservation in filteredRoomBookings" :key="reservation.id">
                        ...
                        <div class="tab-pane fade" :class="{ 'show active': activeTab === 'reserved' }" id="reserved"
                            role="tabpanel" aria-labelledby="reserved-tab">
                            <div class="container-fluid">
                                <table-component :mainHeaders="bookingsOptions" :mainItems="filteredRoomBookings"
                                    :editable="false" :toggleable="false" />
                                <!-- <table class="table" style="table-layout: fixed;word-wrap: break-word;">
                    ...
                    <tbody>
                      <tr v-for="reservation in filteredRoomBookings" :key="reservation.id">
                        ...
                        <div class="tab-pane fade" :class="{ 'show active': activeTab === 'occupied' }" id="occupied"
                            role="tabpanel" aria-labelledby="occupied-tab">
                            <div class="container-fluid">
                                <table-component :mainHeaders="bookingsOptions" :mainItems="filteredRoomBookings"
                                    :editable="false" :toggleable="false" />
                                <!-- <table class="table" style="table-layout: fixed;word-wrap: break-word;">
                ...
                <div class="modal-body">
                    <form @submit.prevent="addWalkInGuest">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control" id="name" v-model="walkinreservation.clientName"
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control" id="address"
                                    v-model="walkinreservation.clientAddress" autocomplete="off" />
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" id="nationality" v-model="walkinreservation.clientNationality"
                                    required>
                                    <option value="">-- Please select --</option>
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" id="clientType" v-model="walkinreservation.clientType"
                                    required>
                                    <option value="">-- Please select --</option>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="row row-cols-1" :class="cartItems.filter((o) => o.type === category.name)
                                                    ...
                                                    : 'row-cols-md-5'
                                                    ">
                                                    <div class="col mb-6" v-for="(item, index) in updatedRooms.filter((r) => {
                                                        if (r.type) {
                                                            const isDataExist = !(
                                        ...
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th scope="col">
                                                        Item
                                                        <span v-if="cartItems.length > 0" class="badge-pill badge-danger">{{
                                                            cartItems.length }}</span>
                                                    </th>
                                                    <th scope="col">
                                                        <span class="d-flex flex-row-reverse">
                                                            <button v-if="cartItems.length > 0" type="button"
                                                                @click="bookAll" class="btn btn-sm bg-primary text-white">
                ...
                <div class="modal-body">
                    ...
                    <form v-else>
                        ...
                        <div class="form-group row mt-2">
                            ...
                            <label for="phone" class="col-sm-2 col-form-label">Phone:</label>
                            <div class="col-sm-4">
                                <input type="tel" class="form-control" id="phone" v-model="reservation.clientPhone"
                                    autocomplete="off" />
                            </div>
                        </div>
                        <div class="form-group row mt-2">
                            <label for="nationality" class="col-sm-2 col-form-label">Nationality:*</label>
                            <div class="col-sm-4">
                                <select class="form-control" id="nationality" v-model="reservation.clientNationality"
                                    required>
                                    <option value="">-- Please select --</option>
                                    <option value="Filipino">Filipino</option>
                                    <option value="Foreign">Foreign</option>
                        ...
                        <div class="form-group row mt-2">
                            ...
                            <div class="col-sm-4">
                                <input v-if="reservation.status !== 'vacant' &&
                                    ...
                                    " type="text" aria-describedby="inputhelp" class="form-control mb-0" id="checkin"
                                    v-model="reservation.checkoutDate" required readonly />
                                <input v-else type="text" aria-describedby="inputhelp" class="form-control mb-0"
                                    id="checkin" :value="setCheckoutDate()" required readonly />
                            </div>
                        </div>
                        <div class="form-group row mt-2">
                            <label for="room" class="col-sm-2 col-form-label">Room:*</label>
                            <div v-if="reservation.status === 'vacant' || toggleselect" class="col-sm-4">
                                <v-select :disabled="roomSelect !== 'ok'" aria-describedby="inputhelp3"
                                    :multiple="!toggleselect" :options="updatedRooms" label="name"
                                    v-model="reservation.roomName" required>
                                ...
                                <small v-if="toggleselect" id="inputhelp3" class="form-text text-muted mt-0">Please select a
                                    new room.</small>
                            </div>
                            ...
                            <div class="col-sm-4">
                                <textarea class="form-control" v-model="reservation.remarks" autocomplete="off"
                                    rows="4"></textarea>
                            </div>
                        </div>
                        <div class="form-group row mt-2">
    ...
    " type="button" class="btn btn-success btn-sm btn-margin rounded">
                                        ...
                                        <button :disabled="disablebutton" v-show="!toggleselect"
                                            v-else-if="reservation.isPaid === 'partial'" @click="
                                                moveToCart();
                                            disablebutton = true;
                                        ...
                                        <button :disabled="disablebutton" v-show="!toggleselect"
                                            v-else-if="reservation.isPaid === 'yes'" @click="
                                                moveToCart();
                                            disablebutton = true;
    ...
    ">
                                ...
                                <button :disabled="disablebutton" @click="
                                    clickTestAddItem();
                                disablebutton = true;
    ...
    " type="button" class="btn btn-primary btn-sm btn-margin rounded">
                                ...
                                <button v-if="new Date().setHours(0, 0, 0, 0) <=
                                    parseDate2(reservation.checkinDate) &&
                                    userdata.role !== 'reservationist' &&
                                    reservation.status !== 'vacant' &&
                                    reservation.status !== 'checkedout' &&
                                    reservation.status !== 'cancelled'
                                    " @click="transferRoom()" type="button"
    ...
    methods: {
      ...
      `,
        ...
        populateCalendarItems() {
            this.calendarItems = this.bookings.map((booking) => {
                ...
                const endDate =
                    ...
                    "-" +
                    booking.checkoutDate.split("/")[0];
                const title = `${booking.room_name}-${booking.groupkey === null
                    ? ""
        ...
        async printSection(idstring, pLength, pWidth, ft) {
            ...
            const bootstrapJS =
                '<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"><script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"><script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js">';
