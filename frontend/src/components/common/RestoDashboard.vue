      ...
      <div class="card x bg-success text-white">
        <div class="card-body row">
          <div class="col-md-8">
            ...
            <p class="mt-0" style="font-size: xx-small">
              in progress / closed order
            </p>
...
export default {
  ...
  data() {
    return {
      ...
      loaded: {},
      pie1Data: {
        labels: ["dine-in", "takeout", "on-hold"],
        datasets: [
          {
            data: [],
          },
  ...
  methods: {
    ...
    pie1Datasets(data) {
      const dinein = data.filter(
        (item) => item.order_type === "dine-in"
      ).length;
      const takeout = data.filter(
        (item) => item.order_type === "take-out"
      ).length;
      const onhold = data.filter(
        (item) => item.order_type === "on-hold"
      ).length;
      this.pie1Data.datasets[0].data = [dinein, takeout, onhold];
    },
    pie2Datasets(data) {
    ...
    pie3Datasets(data) {
      const cash = data
        .filter((item) => item.payMethod === "cash")
        .reduce((accumulator, currentValue) => {
          return accumulator + parseFloat(currentValue.totalPay);
        }, 0);
      const noncash = data
        .filter((item) => item.payMethod === "noncash")
        .reduce((accumulator, currentValue) => {
          return accumulator + parseFloat(currentValue.totalPay);
        }, 0);
    ...
    async loadData() {
      try {
        ...
        this.orderProgress = restoordersData.data.filter(
          (o) =>
            o.status === "progress" &&
            new Date(o.date_created).toLocaleDateString("en-GB") ===
              curday.toLocaleDateString("en-GB")
        ).length;
        this.orderclosed = restoordersData.data.filter(
          (o) =>
            o.status === "closed" &&
            new Date(o.date_created).toLocaleDateString("en-GB") ===
              curday.toLocaleDateString("en-GB")
        ...
        const restotransData = await axios.get(
          this.API_URL + "restotransaction/?N=1000"
        );
        ...
        const summaryOrder = restoordersData.data.filter((item) => {
          const transactionDate = new Date(item.date_created);
          return (
            transactionDate >= new Date(curday.setHours(0, 0, 0, 0)) &&
            transactionDate <
