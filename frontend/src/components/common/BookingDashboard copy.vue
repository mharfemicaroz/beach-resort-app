...
export default {
  ...
  methods: {
    ...
    bar2Datasets(data) {
      ...
      //   return counter;
      // });
      // console.log(this.bar2Data.datasets[0].data);
    },
    ...
    async loadData() {
      try {
        ...
        let roomsData = await axios.get(this.API_URL + "rooms/");
        roomsData = roomsData.data
          .filter((item) => item.isAvailable === true)
          .map((o) => o.name);

        // Fetch reservation stats
        const reservationResponse = axios.get(`${this.API_URL}booking/stats/`, {
          params: {
            column: "id",
            function: "COUNT",
            conditioncolumn1: "checkinDate",
            conditionoperator1: "=",
            conditionvalue1: curday.toLocaleDateString("en-GB"),
          },
        });

        // Fetch available rooms stats
        const availroomsPromises = roomsData.map((o) =>
          axios.get(`${this.API_URL}booking/stats/`, {
            params: {
              column: "id",
              function: "COUNT",
              conditioncolumn1: "room_name",
              conditionoperator1: "=",
              conditionvalue1: o,
              conditioncolumn2: "checkinDate",
              conditionoperator2: "=",
              conditionvalue2: curday.toLocaleDateString("en-GB"),
              conditioncolumn3: "status",
              conditionoperator3: "=",
              conditionvalue3: "checkedin",
            },
          })
        );

        // Fetch transaction item stats
        const transactionItemsResponse = axios.get(
          `${this.API_URL}transaction/item/stats/`,
          {
            params: {
              table: "transactionitem",
              column: "purchaseQty",
              function: "SUM",
              conditioncolumn1: "itemType",
              conditionoperator1: "=",
              conditionvalue1: "ENTRANCE",
              dateconditioncolumn: "dateCreated",
              dateexactvalue: curday.toISOString().split("T")[0], // Format to 'YYYY-MM-DD'
            },
          }
        );

        // Fetch pie chart data
        const statuses = ["cancelled", "reserved", "checkedin", "checkedout"];
        const pieDataPromises = statuses.map((o) =>
          axios.get(`${this.API_URL}booking/stats/`, {
            params: {
              column: "id",
              function: "COUNT",
              conditioncolumn1: "status",
              conditionoperator1: "=",
              conditionvalue1: o,
              conditioncolumn2: "checkinDate",
              conditionoperator2: "=",
        ...
        // Fetch bar chart data by room categories
        const chartDataPromises = this.roomcategories.map((o) =>
          axios.get(`${this.API_URL}booking/stats/`, {
            params: {
              column: "id",
              function: "COUNT",
              conditioncolumn1: "room_type",
              conditionoperator1: "=",
              conditionvalue1: o,
              conditioncolumn2: "checkinDate",
              conditionoperator2: "=",
        ...
        // Fetch user data
        const personData = await axios.get(this.API_URL + "users");
        ...
        // Fetch transaction record stats by person
        const chartDataPromises2 = people.map((o) =>
          axios.get(`${this.API_URL}transaction/record/stats/`, {
            params: {
              column: "cashAmountPay",
              function: "SUM",
              conditioncolumn1: "processedBy",
              conditionoperator1: "=",
              conditionvalue1: o,
              conditioncolumn2: "paymentMethod",
              conditionoperator2: "NOT LIKE",
              conditionvalue2: "%agent%",
              dateconditioncolumn: "transaction_date",
              dateexactvalue: curday.toISOString().split("T")[0], // Format to 'YYYY-MM-DD'
            },
          })
        );

        // Fetch transaction stats by payment method
        const paytypes = ["cash", "non-cash", "agent"];
        const pieDataPromises1 = paytypes.map((o) =>
          axios.get(`${this.API_URL}transaction/record/stats/`, {
            params: {
              column: "cashAmountPay",
              function: "SUM",
              conditioncolumn1: "paymentMethod",
              conditionoperator1: "=",
              conditionvalue1: o,
              dateconditioncolumn: "transaction_date",
              dateexactvalue: curday.toISOString().split("T")[0], // Format to 'YYYY-MM-DD'
        ...
        // Fetch transaction item stats by room categories
        const chartDataPromises3 = this.roomcategories.map((o) =>
          axios.get(`${this.API_URL}transaction/item/stats/`, {
            params: {
              column: "totalCost",
              function: "SUM",
              conditioncolumn1: "itemType",
        ...
        const [
          ...
          reservationResponse,
          Promise.all(availroomsPromises),
          transactionItemsResponse,
          Promise.all(pieDataPromises),
          Promise.all(chartDataPromises),
          Promise.all(chartDataPromises2),
          Promise.all(pieDataPromises1),
          Promise.all(chartDataPromises3),
        ]);

        // Process responses
        this.numReservations = reservationResponseData.data.result;
        this.availableRooms = availroomsResponses.length;
        this.numGuests = transactionItemsResponseData.data.result;
        this.pie1Data.datasets[0].data = pieDataResponses.map(
          (response) => response.data.result
        );
        this.bar1Data.datasets[0].data = chartDataResponses.map(
          (response) => response.data.result
        );

        // Aggregate data by person
        const aggregatedData = chartDataResponses2.reduce(
          (acc, response, index) => {
        ...
        const filteredPeople = Object.keys(aggregatedData);
        const filteredResults = Object.values(aggregatedData);

        this.bar3Data.labels = filteredPeople;
        this.bar3Data.datasets[0].data = filteredResults;

        this.pie2Data.datasets[0].data = pieDataResponses1.map(
          (response) => response.data.result
        );
        this.bar2Data.datasets[0].data = chartDataResponses3.map(
          (response) => response.data.result
        );
